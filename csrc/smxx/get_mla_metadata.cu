// Split-KV metadata scheduler — distributes KV blocks across SM partitions.
//
// Based on FlashMLA (https://github.com/deepseek-ai/FlashMLA)
// Original: github.com/IISuperluminaLII/FlashMLA_Windows_Linux_sm120 (SM120 fork of github.com/deepseek-ai/FlashMLA): csrc/smxx/get_mla_metadata.cu
// License: MIT, Copyright (c) 2025 DeepSeek — see THIRD_PARTY_NOTICES.md
//
// Modifications:
//   - Param struct renamed: GetDecodingMetadataParams → GetMlaMetadataParams
//   - Function renamed: run_get_mla_metadata_kernel (was run_get_mla_metadata_kernel)
//
// This kernel is arch-generic — runs on a single warp (32 threads).
// It reads seqlens_k and distributes work across num_sm_parts partitions,
// producing tile_scheduler_metadata and num_splits arrays.
//
// Supports topk=-1 (use seqlens_k per batch) or topk>0 (uniform for all batches).
// Output layout per partition (TileSchedulerMetaDataSize=8 ints):
...
get_mla_metadata_kernel(__grid_constant__ const GetMlaMetadataParams params) {
    ...
    for (int i = threadIdx.x; i < batch_size; i += 32) {
        ...
        // NOTE if seqlens_k is 0, then first_token_idx == last_token_idx == cur_first_block_idx == cur_last_block_idx == 0.
        // So the sequence will have 1 block. We will correct this later in this kernel.
...
void run_get_mla_metadata_kernel(GetMlaMetadataParams &params, cudaStream_t stream) {
