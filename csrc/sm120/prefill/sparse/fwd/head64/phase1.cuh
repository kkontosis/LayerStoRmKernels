 ...
 *
 * SM120 Absorbed Sparse Prefill (head64) — Pipelined Producer/Consumer
 *
 * Same pipeline as decode: warps 0-3 consume (MMA+softmax), warps 4-7 produce (gather).
 * Simpler than decode: bf16 KV (no FP8 dequant), no split-KV, single query per CTA.
 * Register-resident softmax via fragment_masked_row_max/exp_sum + identity tensor mapping.
 **************************************************************************************************/
...
// Gather one K-tile from bf16 KV (absorbed path, no dequant)
template<int D_QK, typename Traits>
__device__ void gather_k_tile_bf16(
    const cutlass::bfloat16_t* __restrict__ kv_ptr,
    const int* __restrict__ gIndices,
    ...
    for (int i = 0; i < per_thread; ++i) {
        ...
        if (idx < k_tile_elems) {
            ...
            if (sValid[token_row] && global_k < D_QK) {
                int token_index = __ldg(gIndices + k_block * B_TOPK + token_row);
                val = InputT(__ldg(reinterpret_cast<const __nv_bfloat16*>(kv_ptr) + (int64_t)token_index * kv_stride + global_k));
            }
...
sparse_attn_fwd_sm120_kernel(const SparseAttnFwdParams params) {
    using T = Traits<D_QK>;
    using InputT = typename T::InputT;
    ...
    // DETERMINISTIC-only per-warp denominator partials (DET-REDUCE,
    // TD-SPARSE-PREFILL-DETREDUCE — mirrors the dense twin / splitkv_mla.cu).
    ...
    // TD-SPARSE-CHUNK-PREFILL (INV-SPARSE-CHUNK-CAUSAL): per-row causal bound
    // over the staged KV — row s_q_idx attends only selected indices
    // < row_s_kv (chunk-causal sparse prefill; the top-k mask combined with
    // the row's causal prefix bound). nullptr → legacy flat bound s_kv.
    // A fully masked row (row_s_kv == 0 or topk_length == 0) keeps the empty
    // convention: zero output + lse = +inf (finite -1e30 sentinels below).
    const int row_s_kv = params.s_kv_per_row
        ? min(max(__ldg(params.s_kv_per_row + s_q_idx), 0), params.s_kv)
        : params.s_kv;
    ...
    for (int k_block = 0; k_block < num_k_blocks; ++k_block) {
        // Valid mask (producer warps): in-length, non-negative, and inside
        // the row's causal bound (row_s_kv == s_kv when s_kv_per_row null).
        if (is_producer) {
            for (int i = (thread_idx - 128); i < B_TOPK; i += 128) {
                if (i >= 0) {
                    int gidx = k_block * B_TOPK + i;
                    sValid[i] = (gidx < topk_length) && (__ldg(gIndices + gidx) >= 0) && (__ldg(gIndices + gidx) < row_s_kv);
                }
            }
        }
        __syncthreads();
        ...
        if (is_producer) {
            gather_k_tile_bf16<D_QK, T>(kv_ptr, gIndices, sValid, sK_bufs[0],
            ...
            for (int i = 0; i < q_per_thread; ++i) {
                ...
                if (idx < q_tile_elems) {
                    ...
                    sQ(row, col) = (row < params.h_q && col < D_QK) ?
                        InputT(__ldg(reinterpret_cast<const __nv_bfloat16*>(q_ptr) + row * params.stride_q_h_q + col)) : InputT(0);
                }
            }
        }
        __syncthreads();
                ...
                for (int i = 0; i < q_per_thread; ++i) {
                    ...
                    if (idx < q_tile_elems) {
                        ...
                        sQ(row, col) = (row < params.h_q && gk < D_QK) ?
                            InputT(__ldg(reinterpret_cast<const __nv_bfloat16*>(q_ptr) + row * params.stride_q_h_q + gk)) : InputT(0);
                    }
                }
                __syncthreads();
            }

            // Phase B (overlapped): producers gather K[i+1], consumers MMA on K[i]
            if (is_producer && k_tile + 1 < T::NUM_K_TILES_TOTAL) {
                gather_k_tile_bf16<D_QK, T>(kv_ptr, gIndices, sValid,
        ...
        if (thread_idx < B_H) {
            ...
            float rescale;
            if (new_max == NEGATIVE_INFINITY) {
                // All-masked block (every selected index of this B_TOPK
                // group failed the length/validity/causal-bound mask —
                // reachable via s_kv_per_row, TD-SPARSE-CHUNK-PREFILL):
                // carry the running max so state stays consistent for later
                // blocks; rescale 1 keeps sL/rO untouched (exp_sum adds 0).
                // Without this, exp2f((old − (−1e30))·LOG2E) = +inf poisons
                // sL/rO → NaN output.
                sM[thread_idx] = old_max;
                rescale = 1.0f;
            } else {
                rescale = (old_max == NEGATIVE_INFINITY)
        ...
        // Store through swizzled layout matching PV GEMM read path (SmemLayoutVTile).
        ...
        for (int i = 0; i < v_per_thread; ++i) {
            ...
            if (idx < v_elems) {
                ...
                InputT val = InputT(0);
                if (sValid[tr]) val = InputT(__ldg(reinterpret_cast<const __nv_bfloat16*>(kv_ptr) + (int64_t)__ldg(gIndices + k_block * B_TOPK + tr) * kv_stride + vc));
    ...
    if (thread_idx < nvh) {
        ...
        // sM/rS are NATURAL log units here (see dense phase1.cuh note):
        // LSE = m + log(l); no ln2 conversion.
...
void run_fwd_phase1_kernel(const SparseAttnFwdParams& params) {
    ...
    auto kernel = &sparse_attn_fwd_sm120_kernel<D_QK, DETERMINISTIC>;
