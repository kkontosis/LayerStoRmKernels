// Sparse prefill parameter structs.
// Originally from github.com/deepseek-ai/FlashMLA: csrc/params.h (MIT License,
// Copyright (c) 2025 DeepSeek — see THIRD_PARTY_NOTICES.md).
// Copied here so csrc/ is self-contained with no ref/ dependencies.
...
struct SparseAttnFwdParams {
    ...
    // TD-SPARSE-CHUNK-PREFILL: per-query-row causal bound over the shared
    // staged KV. non-null: query row i attends only selected indices in
    // [0, min(s_kv_per_row[i], s_kv)) — the top-k index mask COMBINED with
    // row i's causal prefix bound (chunk-causal sparse prefill; mirrors
    // DenseAttnFwdParams::s_kv_per_row, INV-SPARSE-CHUNK-CAUSAL). Row i's
    // out/lse are then bit-identical to an s_q==1 call with
    // s_kv == s_kv_per_row[i] over the same staging + index row.
    // nullptr → legacy flat bound: every row attends any selected index < s_kv.
    const int* __restrict__ s_kv_per_row = nullptr;   // [s_q], may be nullptr
    ...
    // reduction (DET-REDUCE, TD-SPARSE-PREFILL-DETREDUCE). false (default) =
    // legacy cross-warp atomicAdd path (byte-identical to pre-flag builds).
