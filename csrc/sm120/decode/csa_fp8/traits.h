...
//==============================================================================
// SM120 CSA FP8 Decode — Traits (Dual MMA)
//
// Adapted from sparse_fp8/traits.h for V4 CSA decode.
// Same dual-MMA approach: FP8 for NOPE (SM89_16x8x32), BF16 for ROPE (SM80_16x8x16).
// Addition: smem_V_scales for V4's separate V quantization scale.
//
// Smem budget: ~55KB << 99KB (SM120 limit)
//==============================================================================
...
struct Traits {
    using Cfg = Config;
    using InputT = cutlass::float_e4m3_t;    // FP8 e4m3 for K_NOPE, V_NOPE, Q_NOPE (in-kernel cast)
    ...
    static constexpr int NUM_NOPE_TILES = Cfg::NUM_NOPE_TILES;
    ...
    //==========================================================================
    // Shared Memory Plan — Dual MMA (~55KB << 99KB)
    //
    // Same as SnapMLA sparse_fp8 + smem_V_scales for V4's separate V scale.
    //==========================================================================
    struct SharedMemoryPlan {
        ...
        // Per-token K quantization scales (reused for SWA phase)
        cute::array_aligned<float, TOPK_BLOCK_SIZE> smem_K_scales;          // 256B
        // Per-token V quantization scales (V4: separate from K_scale)
        cute::array_aligned<float, TOPK_BLOCK_SIZE> smem_V_scales;          // 256B
